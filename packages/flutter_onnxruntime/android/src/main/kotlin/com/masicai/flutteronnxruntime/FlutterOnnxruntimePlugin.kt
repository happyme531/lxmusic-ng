            ...
            when (call.method) {
                        ...
                        for (provider in providers) {
                            ...
                            when (provider) {
                                ...
                                "ARM_NN" -> {
                                    result.error(
                                        "INVALID_PROVIDER",
                                        "ARM_NN is not available in ONNX Runtime 1.28.0",
                                        null,
                                    )
                                    return
                                }
                ...
                "runInference" -> {
                    try {
                        ...
                        val requestedOutputs = call.argument<List<String>>("requestedOutputs")?.toSet()
                        val pinnedOutputs = call.argument<Map<String, Any>>("pinnedOutputs")
                        ...
                        val ortPinnedOutputs = HashMap<String, OnnxValue>()
                        ...
                        try {
                            ...
                            for ((name, value) in pinnedOutputs.orEmpty()) {
                                if (value is Map<*, *> && value.containsKey("valueId")) {
                                    val valueId = value["valueId"] as String
                                    val existingTensor = ortValues[valueId]
                                    if (existingTensor != null) {
                                        ortPinnedOutputs[name] = existingTensor
                                    } else {
                                        result.error(
                                            "INVALID_ORT_VALUE",
                                            "Pinned output OrtValue with ID $valueId not found",
                                            null,
                                        )
                                        return
                                    }
                                    ...
                                    result.error(
                                        ...
                                        "Pinned output for '$name' must be an OrtValue reference with value ID",
                                        null,
                                    )
                                    return
                                }
                            ...
                            val usesPinnedOrRequestedOutputs =
                                requestedOutputs != null || ortPinnedOutputs.isNotEmpty()
                            val allocatedOutputNames =
                                requestedOutputs
                                    ?: session.outputNames.filterNot(ortPinnedOutputs::containsKey).toSet()

                            // Run inference with correctly typed inputs and optional preallocated outputs.
                            val ortOutputs =
                                if (usesPinnedOrRequestedOutputs) {
                                    if (ortRunOptions != null) {
                                        session.run(
                                            runInputs,
                                            allocatedOutputNames,
                                            ortPinnedOutputs,
                                            ortRunOptions,
                                        )
                                    } else {
                                        session.run(runInputs, allocatedOutputNames, ortPinnedOutputs)
                                    }
                                } else {
                                    if (ortRunOptions != null) {
                            ...
                            val returnedOutputNames =
                                if (usesPinnedOrRequestedOutputs) {
                                    allocatedOutputNames
                                } else {
                                    session.outputNames
                                }
                            for (outputName in returnedOutputNames) {
                ...
                "createZeroOrtValue" -> {
                    try {
                        ...
                        if (dataType == null || shape == null || shape.any { it < 0 }) {
                            result.error("INVALID_ARG", "Missing data type or invalid shape", null)
                            return
                        }

                        val longShape = shape.map { it.toLong() }.toLongArray()
                        val elementCount =
                            shape.fold(1L) { product, dimension ->
                        ...
                        val bytesPerElement =
                            when (dataType) {
                                ...
                                "uint8", "bool" -> 1
                                    ...
                                    result.error(
                                        "UNSUPPORTED_TYPE",
                                        "Unsupported zero tensor data type: $dataType",
                                        null,
                                    )
                                    return
                                }
                            }
                        val byteCount = Math.multiplyExact(elementCount, bytesPerElement.toLong())
                        if (byteCount > Int.MAX_VALUE) {
                            result.error("TENSOR_TOO_LARGE", "Tensor allocation exceeds 2 GiB", null)
                            return
                        }
                        val buffer =
                            ByteBuffer.allocateDirect(byteCount.toInt())
