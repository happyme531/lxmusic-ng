...
class OrtValue {
  ...
  /// Creates a zero-filled tensor in native memory.
  ///
  /// This avoids transferring large initialization buffers over a platform
  /// channel and is intended for reusable state such as KV caches.
  static Future<OrtValue> zeros(OrtDataType dataType, List<int> shape) async {
    _calculateExpectedElements(shape);
    final result = await FlutterOnnxruntimePlatform.instance.createZeroOrtValue(
      dataType.name,
      shape,
    );
    return OrtValue.fromMap(result);
