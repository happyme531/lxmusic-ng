...
class OrtSession {
  ...
  Future<Map<String, OrtValue>> run(
    ...
    final overlap = requestedOutputs?.intersection(
      pinnedOutputs?.keys.toSet() ?? const <String>{},
    );
    if (overlap != null && overlap.isNotEmpty) {
      throw ArgumentError(
        'Outputs cannot be both requested and pinned: ${overlap.join(', ')}',
      );
    }
