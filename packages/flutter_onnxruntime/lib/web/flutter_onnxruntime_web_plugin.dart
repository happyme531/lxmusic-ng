...
class FlutterOnnxruntimeWebPlugin extends FlutterOnnxruntimePlatform {
  ...
  Future<Map<String, dynamic>> runInference(
    ...
    try {
      ...
      // Run inference
      final runPromise = jsRunOptions != null
    ...
    try {
      ...
      if (session.has('metadata')) {
        ...
        if (metadata.has('producerName')) {
          metadataMap['producerName'] = metadata
              .getProperty('producerName'.toJS)
              .toString();
        }

        if (metadata.has('graphName')) {
          metadataMap['graphName'] = metadata
              .getProperty('graphName'.toJS)
              .toString();
        }

        if (metadata.has('domain')) {
          metadataMap['domain'] = metadata
              .getProperty('domain'.toJS)
              .toString();
        }
